...
export async function POST(request: Request) {
  ...
  let fixedUrl = productUrl;

  if (productUrl.includes("Me mande ofertas para este produto ")) {
    productUrl = productUrl.replace("Me mande ofertas para este produto ", "");
    fixedUrl = fixedUrl.replace("Me mande ofertas para este produto ", "");
  }
  ...
  try {
    if (!productUrl.includes("http")) {
      ...
      const html = await scraper.get(fixedUrl, options);
      const $ = cheerio.load(html.data);
      ...
      source = "Tok&Stok";
      img = $("[property='og:image']").attr("content");
      title = $("[property='og:title']").attr("content")?.slice(0, -10);
      price = String(
      ...
      title = $("[property='og:title']").attr("content");
      price = $(
        "#control-box-content > div:nth-child(2) > div.cav--c-lesPJm.cav--c-lesPJm-ieYUIHI-css > div.cav--c-lesPJm.cav--c-lesPJm-iBEpdf-css > div > div.cav--c-lesPJm.cav--c-lesPJm-ifGHEql-css > div.cav--c-lesPJm.cav--c-bsUccK.cav--c-bsUccK-ijaXjWv-css > div > div.cav--c-gqwkJN.cav--c-gqwkJN-iustkc-css > div > div:nth-child(1) > span"
      )
        .text()
        .trim()
        .slice(3, -3);
      ...
      title = $("[property='og:title']").attr("content")?.split("|")[0] || null;
      price =
        String(
          Math.floor(
            Number($("[property='product:price:amount']").attr("content"))
          )
        ) ||
        String(
